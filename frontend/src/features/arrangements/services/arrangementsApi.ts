import { RequestRecord, ManagerRequest } from './arrangementRequestsApi';
...
const Page = (audience: 'manager' | 'viewer') => z.object({
  organization_id: z.uuid(),
  items: z.array(z.union([...(audience === 'manager' ? [ManagerRequest] : []), RequestRecord, z.object({ id: z.uuid(), name: z.string().min(1).max(200), status: Status }).strict()])).max(100),
