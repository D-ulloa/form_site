...
function inquilino(slug: OrganizationSlug = 'azar'): ReturnType<typeof organizationContext> {
  const context = organizationContext(slug);
  return { ...context, membership: { ...context.membership, role: 'inquilino' },
    capabilities: ['inquilino.home.read'], home_destination: 'inquilino' };
}
...
beforeEach(() => {
  ...
  contextStatus = 200;
  resolveContext = async slug => inquilino(slug === 'solar' ? 'solar' : 'azar');
...
it.each(['', '/inquilino', '/arrangements', '/properties/new', '/properties/success/submission',
  ...
  expect(window.location.pathname).toBe('/t/azar/inquilino');
  assertEmptyHome();
});
...
it('isolates different roles in A and B and removes cached product data', async () => {
  resolveContext = async slug => slug === 'solar' ? organizationContext('solar') : inquilino();
  mount('/t/solar');
  await screen.findByRole('button', { name: /^Agregar nueva propiedad/u });
...
it.each([401, 403, 404, 503])('fails closed on context HTTP %s', async code => {
  contextStatus = code; mount('/t/azar/inquilino');
...
it.each(['suspended', 'removed'] as const)('rejects a returned %s membership even with stale capabilities', async status => {
  resolveContext = async () => ({ ...inquilino(), membership: { ...inquilino().membership, status } });
  mount('/t/azar/inquilino');
...
it('requires the home capability and server destination together', async () => {
  resolveContext = async () => ({ ...inquilino(), capabilities: [], home_destination: null });
  mount('/t/azar/inquilino');
  await screen.findByRole('heading', { name: 'Elegí una organización' });
...
it.each(['focus', 'visibilitychange', 'navigation'] as const)('rechecks suspension on %s', async event => {
  mount('/t/azar/inquilino'); await screen.findByRole('heading', { name: 'Inicio' });
  contextStatus = 404;
  act(() => {
...
it('rechecks a role change within the same session when authentication refreshes', async () => {
  ...
  resolveContext = async () => inquilino();
  session = structuredClone(organizationSession);
...
it('discards an old context response after switching organizations', async () => {
  ...
  resolveContext = slug => slug === 'solar' ? Promise.resolve(inquilino('solar'))
    : new Promise(resolve => { release = resolve; });
  mount('/t/azar');
  await waitFor(() => expect(release).toBeDefined());
  navigate('/t/solar/inquilino');
  await screen.findByRole('heading', { name: 'Inicio' });
  await act(async () => release(organizationContext('azar')));
  expect(window.location.pathname).toBe('/t/solar/inquilino');
  assertEmptyHome();
});

it('rejects context for another identity', async () => {
  resolveContext = async () => ({ ...inquilino(), membership: { ...inquilino().membership, user_id: 'someone-else' } });
  mount('/t/azar/inquilino');
  await screen.findByRole('heading', { name: 'Elegí una organización' });
});
