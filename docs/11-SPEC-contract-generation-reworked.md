...
This spec extends `docs/10-SPEC-contract-generation-reworked.md` with the next contract-generation iteration for the client-side intake form. It defines enhancements to the `Inquilino` and `Garantes` sections, adds DNI image upload support for each tenant and guarantor, removes the `Aprobar Contrato` field from both backend and frontend, and codifies computed `Formateada_1` and `Formateada_2` field behavior.
...
- Allow each `Inquilino` and each `Garante` entry to upload up to two DNI image files: one front image and one back image.
- Remove the `Aprobar Contrato` field entirely from both frontend forms and backend validation.
- Convert `Ajuste` into a dropdown with exactly two options: `IPC` and `IPL`.
...
- `Ajuste`
  - Render as a dropdown/select input with the options `IPC` and `IPL`.
  - Validate backend submissions to accept only `IPC`, `IPL`, or an absent value if the field is optional.
  - Do not allow free-text input for this field.

- `DNI uploads`
  - Each `Inquilino` and each `Garante` entry may upload up to two image files representing DNI documentation.
  - Exactly two files are allowed per repeatable block: one labeled `Frente DNI` and one labeled `Dorso DNI`.
  - The backend must enforce the maximum count of two files per tenant/guarantor entry and validate file types as images.
...
- Each tenant and guarantor block must include file upload controls for DNI images:
  - one upload control labeled `Frente DNI`
  - one upload control labeled `Dorso DNI`
  - enforce a maximum of two files per block.
- `Ajuste` must render as an explicit select/dropdown control with `IPC` and `IPL`.
...
- If the form is rendered in read-only mode after submission, all fields remain visible but not editable.
...
4. Each `Inquilino` and each `Garante` block can upload exactly two DNI images: one front image and one back image.
5. The `Aprobar Contrato` field is absent from frontend forms and backend schema validation.
6. `Ajuste` is a dropdown with only `IPC` and `IPL` values.
...
- UI test validating that `Ajuste` only accepts `IPC` or `IPL`.
- UI test validating DNI image upload controls for each tenant and guarantor block, with a maximum of two files.
