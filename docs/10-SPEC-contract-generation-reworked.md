...
- `status` TEXT CHECK(status IN ('open','complete','archived')) DEFAULT 'open'
...
- Both form pages are hosted by the app and remain accessible until `contract_entries.status` transitions to `archived` or `complete`. `complete` keeps the pages accessible in read-only mode; tokens may be disabled after administrative review.
...
1. Apply `backend/supabase/migrations/20260724000000_contract_entries.sql` to the target Supabase project.
