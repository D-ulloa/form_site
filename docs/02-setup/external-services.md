...
Saved contract JSON contains bucket/path metadata, not a public URL or an expiring download URL. The bucket remains private. The SPEC-13 administrator detail read validates media references against the selected entry and configured bucket, then returns short-lived signed view URLs in its normalized inspection model; callers cannot provide arbitrary storage paths to that flow.
