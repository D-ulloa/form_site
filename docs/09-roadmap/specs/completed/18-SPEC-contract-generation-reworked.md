...
- The editable role workflow now provides this post-submission recovery path.
...
- This document records the frontend behavior implemented in the current contract-generation UI.
- The implemented changes improve clarity and recovery in the UI while preserving the existing backend validation and storage behavior.
- Future changes should keep frontend copy and backend validation changes scoped separately.
