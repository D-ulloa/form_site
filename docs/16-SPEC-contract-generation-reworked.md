...
- Enable every `Garante` on the client form to submit both `Frontal` and `Dorso` DNI images, consistent with `Inquilino` behavior.
...
- All buttons used to upload files and DNI images in the contract generation workflow must show a hover effect.
...
- Every `Garante` block in the client form must support submitting two DNI images: `Frontal` and `Dorso`.
...
3. Each `Garante` in the client form can submit both `Frontal` and `Dorso` DNI images.
