...
- The editable form is locked during that save sequence. Once an upload succeeds, its `File` value is promoted to the stable reference in form state, so a retriable final-submit failure does not upload the object again. An ambiguous final response triggers a schema refresh and switches to read-only if the server already committed the submission.
- The `contract-evidence` bucket is private and separate from the image-only SPEC-11 DNI bucket. Its MIME allowlist is exactly `application/pdf`, `image/jpeg`, `image/png`, `image/gif`, `image/webp`, `image/bmp`, and `image/tiff`.
...
- Focused backend coverage lives in `backend/tests/contract-entries-spec14.test.ts`; frontend coverage lives in the contract file-receiver, repeatable-section, hosted-form, and inspection tests.
...
- Apply `backend/supabase/migrations/20260729000000_contract_spec14.sql` after the SPEC-10 and SPEC-11 migrations.
