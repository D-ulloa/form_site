-- Standalone PostgreSQL ONLY: an empty disposable database named spec45*.
-- A real Supabase test instance already has Auth/roles; apply the selected migrations there instead.
\set ON_ERROR_STOP on
do $$ begin
  if current_database() not like 'spec45%' or exists(select from pg_tables where schemaname in ('public','auth')) then
    raise exception 'An empty disposable spec45* database is required';
  end if;
