-- Standalone PostgreSQL ONLY: an empty disposable database named spec46*.
-- A real Supabase test instance already has Auth/roles; apply the selected migrations there instead.
\set ON_ERROR_STOP on
do $$ begin
  if current_database() not like 'spec46%' or exists(select from pg_tables where schemaname in ('public','auth')) then
    raise exception 'An empty disposable spec46* database is required';
  end if;
...
\ir ../migrations/20260912160000_spec45_tenant_contact.sql
\ir ../migrations/20260912170000_spec45_personal_assignments.sql
\ir ../migrations/20260922120000_spec46_work_reports.sql
