-- Standalone PostgreSQL ONLY: an empty disposable database named spec43*.
-- A real Supabase test instance already has Auth/roles; apply the selected migrations there instead.
\set ON_ERROR_STOP on
do $$ begin
  if current_database() not like 'spec43%' or exists(select from pg_tables where schemaname in ('public','auth')) then
    raise exception 'An empty disposable spec43* database is required';
  end if;
...
\if :{?spec43_before_upgrade}
-- Leave the old order schema for the dedicated upgrade/preflight test.
\else
