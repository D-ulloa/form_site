-- Synthetic identities only; install once in a disposable SPEC-45 database.
do $$ begin
  if current_database() not like 'spec45%' then raise exception 'Disposable spec45 database required'; end if;
end; $$;
...
select exists(select 1 from information_schema.columns where table_name='organization_memberships' and column_name='inquilino_contact_number') as has_contact \gset
\if :has_contact
insert into public.organization_memberships(id,organization_id,user_id,role,status,joined_at,arrangement_property_id,inquilino_contact_number)
 ...
 (public.spec42_create_arrangement_property('20000000-0000-4000-8000-000000000001','30000000-0000-4000-8000-000000000001','Casa de prueba','spec44-property','spec44-fixtures')->>'id')::uuid,'+58 0412 1234567');
\else
...
begin
 if current_database() not like 'spec45%' then raise exception 'Disposable database required'; end if;
 if p_activate then
...
insert into public.user_profiles(user_id,display_name,locale,time_zone)
 select id,split_part(email,'@',1),'es','America/Caracas' from auth.users where id::text>='10000000-0000-4000-8000-000000000013';
...
\else
insert into public.organization_memberships(id,organization_id,user_id,role,status,joined_at,arrangement_property_id)
select '30000000-0000-4000-8000-000000000015',organization_id,'10000000-0000-4000-8000-000000000015','inquilino','active',now(),arrangement_property_id
from public.organization_memberships where id='30000000-0000-4000-8000-000000000010';
\endif
