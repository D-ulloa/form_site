...
export async function validateWebhookDestination(raw, resolve) {
    let url;
    try {
        url = new URL(raw);
    }
    ...
    if (url.protocol !== 'https:' || url.username || url.password || url.hash
        || (url.port && url.port !== '443') || raw.length > 2048)
        throw new Error('UNSAFE_DESTINATION');
...
export function serializeWebhookEnvelope(envelope) {
...
export function signWebhook(body, timestamp, eventId, secret) {
    return `v1=${createHmac('sha256', secret).update(timestamp).update('.').update(eventId).update('.').update(body).digest('hex')}`;
}
export function verifyWebhookSignature(signature, expected) {
