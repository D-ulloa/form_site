...
export function validateIntegrationConfiguration(provider, purpose, value) {
    if (!purposes[provider].has(purpose))
        throw new Error('INVALID_INTEGRATION_CONFIGURATION');
    return Object.freeze(schemas[provider].parse(value));
}
export function safeIntegrationProjection(row) {
