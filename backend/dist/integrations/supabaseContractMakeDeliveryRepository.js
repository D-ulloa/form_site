...
function eventFrom(value) {
    ...
    const organizationReference = text(row?.organization_reference);
    const resourceId = text(row?.resource_id);
    const resourceVersion = integer(row?.resource_version);
    ...
    if (!row || !data || !eventId || !eventType || row.schema_version !== '1' || !organizationReference
        || !resourceId || resourceVersion === null || !occurredAt || !idempotencyKey)
        return null;
...
function deliveryFrom(value) {
    const row = object(value);
    const event = eventFrom(row?.event);
    const id = text(row?.id);
    const organizationId = text(row?.organization_id);
    const outboxEventId = text(row?.outbox_event_id);
    const integrationId = text(row?.integration_id);
    const leaseToken = text(row?.lease_token);
    const leaseExpiresAt = text(row?.lease_expires_at);
    const idempotencyKey = text(row?.idempotency_key);
    ...
    if (!row || !event || !id || !organizationId || !outboxEventId || !integrationId || row.provider !== 'make_webhook'
        ...
        || !idempotencyKey || attemptCount === null || version === null)
        throw new Error('INVALID_LEASED_DELIVERY');
...
function outcomeFields(outcome) {
    if (outcome.kind === 'succeeded')
        return { error: null, externalId: outcome.external_id, receipt: outcome.receipt_reference ?? null };
...
export function createSupabaseContractMakeDeliveryRepository(client) {
    const processingVersions = new Map();
    async function transition(delivery, expectedVersion, nextState, safeErrorCode, externalId, receiptReference, nextAttemptAt) {
    ...
    return {
        ...
        async resolveExecutionContext(delivery) {
            ...
            if (error || !row || !configuration || configurationVersion === null)
                return null;
            return { scope: createOrganizationScope(delivery.organization_id), integration_id: delivery.integration_id,
                ...
                credential_version: credentialVersion, configuration };
        },
        ...
        async finish(delivery, outcome, decision, nextAttemptAt) {
            ...
            const version = processingVersions.get(delivery.id);
            if (version === undefined)
                throw new Error('DELIVERY_NOT_STARTED');
            try {
...
//# sourceMappingURL=supabaseContractMakeDeliveryRepository.js.map