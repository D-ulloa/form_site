...
export async function cleanupArrangementAssets(client, storage, scope) {
    const requestId = randomUUID();
    const call = async (action, id, result) => {
    ...
    for (const item of items) {
        let result;
        try {
            result = await storage.remove(item.bucket_name, item.object_path) === 'not_found' ? 'not_found_reconciled' : 'deleted';
        }
        catch {
            result = 'failed';
        }
...
//# sourceMappingURL=arrangementCleanup.js.map