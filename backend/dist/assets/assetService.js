...
export function createAssetService(dependencies) {
    ...
    return {
        async initialize(context, input) {
            ...
            if (!Number.isSafeInteger(totalBytes) || (input.owner_type === 'arrangement_order' && totalBytes > 1024 ** 3))
            ...
            const uploads = session.state === 'consumed' ? [] : await Promise.all(intents.map(async (intent) => {
                ...
                await dependencies.repository.recordUrlIssued(context.scope, sessionId, intent.id, input.owner_type === 'arrangement_order'
