import { isValidContractGoogleFormLink, isValidContractSheetName, isValidContractSpreadsheetId, } from './contractEnvironmentValidation.js';
...
const sections = [
    ...
    },
    {
        ...
        fields: [
            ...
            { name: 'guarantor_company_registration', label: 'Número de Matrícula de la Empresa', type: 'string', required: false, sensitive: true },
...
const columnMap = Object.fromEntries(orderedFields.map((field) => [field.name, field.label]));
const rentContractSchema = {
...
export class ContractConfigurationError extends Error {
    ...
    constructor(missingVariables) {
        super(`Contract generation is not configured. Set ${missingVariables.join(', ')} and restart the backend.`);
...
export function getContractSchemaDefinition(schemaId) {
...
export function getPublicContractSchema(schemaId, environment = process.env) {
    const schema = getContractSchemaDefinition(schemaId);
    const values = requireEnvironmentValues(['CONTRACT_GOOGLE_FORM_LINK'], environment);
...
export function getContractSchemaConfig(schemaId, environment = process.env) {
    const schema = getContractSchemaDefinition(schemaId);
...
//# sourceMappingURL=contractSchemas.js.map