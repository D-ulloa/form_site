import { EXTENSION_MODULE_KEYS } from './types.js';
const modules = new Set(EXTENSION_MODULE_KEYS);
const transitions = {
...
export function isExtensionModuleKey(value) {
    return modules.has(value);
}
export function assertExtensionModuleTransition(from, to) {
 ...
 */
export function evaluateExtensionAccess(input) {
    ...
    if (input.entitled !== true)
        return { allowed: false, code: 'FEATURE_NOT_ENABLED' };
    if (input.within_quota === false)
        return { allowed: false, code: 'QUOTA_EXCEEDED' };
    return { allowed: true };
}
//# sourceMappingURL=moduleGate.js.map