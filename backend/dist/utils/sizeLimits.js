...
export const MAX_MEDIA_FILES = Number.isFinite(configuredMaxMediaFiles) && configuredMaxMediaFiles > 0
...
export function validateMediaUploadDescriptors(uploads) {
    if (uploads.length === 0)
        return true;
    if (uploads.length > MAX_MEDIA_FILES)
        return false;
...
export function getUploadDescriptorTotalSize(uploads) {
