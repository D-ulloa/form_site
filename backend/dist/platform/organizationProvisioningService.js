...
function receipt(operation, email, result) {
...
export class OrganizationProvisioningService {
    ...
    organizations;
    environment;
    constructor(repository, identity, identityAdmin, organizations, environment = process.env) {
    ...
    async preflight(manifest) {
        ...
        if (!database.operator_eligible)
            throw new OrganizationProvisioningError('FORBIDDEN');
        let owners;
        try {
            owners = await this.identityAdmin.resolveByEmail(manifest.initial_owner.email);
        }
        catch {
            throw new OrganizationProvisioningError('OWNER_PROVISIONING_FAILED');
        }
        const blockers = [];
        if (!database.slug_available)
            blockers.push('SLUG_CONFLICT');
        if (database.migration_conflict)
            blockers.push('MIGRATION_INVENTORY_CONFLICT');
        if (owners.length > 1)
            blockers.push('OWNER_IDENTITY_AMBIGUOUS');
        if (owners.length === 1 && !owners[0]?.eligible)
            blockers.push('OWNER_IDENTITY_INELIGIBLE');
        ...
        const ownerAction = owners.length === 0 ? 'create_activation_required'
            : owners[0]?.activation_required ? 'reuse_activation_required' : 'reuse_active';
        return { target, blockers, ownerAction };
    }
    async dryRun(manifest) {
    ...
    async execute(manifest, expectedFingerprint) {
        assertOrganizationProvisioningEnabled(this.environment);
        const fingerprint = manifestFingerprint(manifest);
        if (!/^[0-9a-f]{64}$/u.test(expectedFingerprint))
            throw new OrganizationProvisioningError('FINGERPRINT_REQUIRED');
        if (fingerprint !== expectedFingerprint)
            throw new OrganizationProvisioningError('FINGERPRINT_MISMATCH');
        const target = resolveOrganizationProvisioningTarget(this.environment);
        if (target.approval_reference !== manifest.approval_reference) {
            throw new OrganizationProvisioningError('APPROVAL_REQUIRED');
        }
        ...
        if (!database.operator_eligible)
            throw new OrganizationProvisioningError('FORBIDDEN');
        if (database.migration_conflict)
            throw new OrganizationProvisioningError('MIGRATION_INVENTORY_CONFLICT');
        ...
        let ownerMatches;
        try {
            ownerMatches = await this.identityAdmin.resolveByEmail(manifest.initial_owner.email);
        }
        catch {
            throw new OrganizationProvisioningError('OWNER_PROVISIONING_FAILED');
        }
        if (ownerMatches.length > 1)
            throw new OrganizationProvisioningError('OWNER_IDENTITY_AMBIGUOUS');
        ...
        try {
            ...
            try {
                ...
                if (reconciliationError instanceof OrganizationProvisioningError
                    && reconciliationError.code !== 'READBACK_FAILED')
...
//# sourceMappingURL=organizationProvisioningService.js.map