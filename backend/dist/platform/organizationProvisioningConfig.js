...
export function resolveOrganizationProvisioningTarget(environment) {
    ...
    let actualProject = '';
    try {
        actualProject = new URL(environment.SUPABASE_URL ?? '').hostname.split('.')[0] ?? '';
    }
    catch { /* fail below */ }
...
export function assertOrganizationProvisioningEnabled(environment) {
    if (environment.ORGANIZATION_PROVISIONING_ENABLED !== 'true') {
        throw new OrganizationProvisioningError('PROVISIONING_DISABLED');
    }
}
//# sourceMappingURL=organizationProvisioningConfig.js.map