import { z } from 'zod';
import { ArrangementStatus, RequestRecord, WorkReport, ArrangementRequestError } from '../arrangements/requestRepository.js';
...
export function arrangementReadCapability(audience) {
...
export function createArrangementAssignmentsService(repository, storage, environment) {
    function authority(scope, actor, audience, capability = arrangementReadCapability(audience)) {
        if (arrangementAudience(actor) !== audience)
            throw new ArrangementRequestError('FORBIDDEN', 403);
        requireArrangementAuthority(scope, actor, capability);
        if (audience === 'tenant' && !actor.membership.arrangement_property_id)
            throw new ArrangementRequestError('PROPERTY_REQUIRED', 409);
    }
    const schema = (audience) => audience === 'manager' ? ManagerRequest : audience === 'personal' ? PersonalRequest : RequestRecord;
    const call = (scope, actor, action, input) => repository.call(scope, actor, `v3.${action}`, input);
    const callReport = (scope, actor, action, input) => repository.call(scope, actor, `v4.${action}`, input);
    return {
        async listAssigned(scope, actor, audience, raw) {
        ...
        async assignees(scope, actor, raw) {
            ...
            const query = Query.omit({ status: true }).parse(raw);
            ...
            const after = query.cursor ? codec.decode(query.cursor) : null;
            const page = parse(z.object({ organization_id: z.literal(scope.organization_id), items: z.array(Candidate).max(limit + 1) }).strict(), await call(scope, actor, 'internal.assignees', { limit, after_id: after?.id }));
        ...
        async detail(scope, actor, audience, orderId) {
            authority(scope, actor, audience);
            return parse(schema(audience).extend({ id: z.literal(z.uuid().parse(orderId)), organization_id: z.literal(scope.organization_id) }), await call(scope, actor, `${prefix(audience)}.detail`, { order_id: orderId }));
        },
        async assignedView(scope, actor, audience, orderId, assetId) {
            authority(scope, actor, audience);
            const record = parse(z.object({ id: z.literal(z.uuid().parse(assetId)), organization_id: z.literal(scope.organization_id), state: z.literal('attached'),
                bucket_name: z.literal('arrangement-media'), object_path: z.string(), display_filename: z.string() }).passthrough(), await call(scope, actor, `${prefix(audience)}.view`, { order_id: z.uuid().parse(orderId), asset_id: assetId }));
            return storage.issueView(record.bucket_name, record.object_path, 60, record.display_filename);
        },
        async mutateAssignment(scope, actor, orderId, action, raw) {
            ...
            return parse(ManagerRequest.extend({ id: z.literal(z.uuid().parse(orderId)), organization_id: z.literal(scope.organization_id) }), await call(scope, actor, `internal.${action}`, { ...body, order_id: orderId }));
        },
        async changes(scope, actor, audience) {
            authority(scope, actor, audience);
            return parse(z.object({ revision: z.string().regex(/^\d+$/u) }).strict(), await call(scope, actor, `${prefix(audience)}.changes`, {}));
        },
        async saveWorkReport(scope, actor, orderId, raw) {
            ...
            return parse(WorkReportMutation.extend({ id: z.literal(z.uuid().parse(orderId)) }), await callReport(scope, actor, 'personal.report.save', { ...body, order_id: orderId }));
        },
        async submitWorkReport(scope, actor, orderId, raw) {
            authority(scope, actor, 'personal', 'personal.arrangements.report.write');
            const body = z.object({ expected_version: Version, report_version: Version }).strict().parse(raw);
            return parse(WorkReportMutation.extend({ id: z.literal(z.uuid().parse(orderId)) }), await callReport(scope, actor, 'personal.report.submit', { ...body, order_id: orderId }));
        },
        async acceptWorkReport(scope, actor, orderId, raw) {
            authority(scope, actor, 'tenant', 'inquilino.arrangements.accept');
            const body = z.object({ expected_version: Version }).strict().parse(raw);
            return parse(RequestRecord.extend({ id: z.literal(z.uuid().parse(orderId)), organization_id: z.literal(scope.organization_id) }), await callReport(scope, actor, 'inquilino.accept', { ...body, order_id: orderId }));
        },
    };
}
//# sourceMappingURL=arrangementAssignments.js.map