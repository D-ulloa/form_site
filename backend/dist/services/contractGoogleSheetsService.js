...
import { GoogleServiceAccountConfigurationError, createGoogleServiceAccountAuth, } from '../utils/googleServiceAccountAuth.js';
import { ContractSheetsAppendError, getGoogleSheetsErrorStatus, isRetriableGoogleSheetsError, } from './contractSheetsErrors.js';
import { assertContractSheetHeaders, buildContractSheetHeaderReadRequest, } from './contractSheetHeaderValidation.js';
const SHEETS_SCOPES = ['https://www.googleapis.com/auth/spreadsheets'];
export function buildContractSheetAppendRequest(input) {
...
async function executeGoogleHeaderRead(request) {
...
async function executeGoogleAppend(request) {
    const auth = createGoogleServiceAccountAuth(SHEETS_SCOPES);
    const sheets = google.sheets({ version: 'v4', auth });
...
export async function appendContractSheetRow(input, overrides = {}) {
    const dependencies = {
...
//# sourceMappingURL=contractGoogleSheetsService.js.map