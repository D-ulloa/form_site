...
function serializeCookie(value, environment, rememberMe) {
    const attributes = [
    ...
    if (rememberMe)
        attributes.push(`Max-Age=${30 * 24 * 60 * 60}`);
...
export function serializeContractPasswordSessionCookie(session, environment = process.env, rememberMe = false) {
    ...
    return serializeCookie(signedValue(encode({ ...session, expiresAt }), environment), environment, rememberMe);
}
...
function isAdminMetadata(value) {
...
async function makeSession(client, user) {
    ...
    if (!user.id || !email)
        throw new ContractPasswordAuthError('invalid_credentials', 'La cuenta no tiene un email válido.');
