import { createArrangementPersonalRouter } from './arrangementPersonal.js';
import { createArrangementRequestsRouter } from './arrangementRequests.js';
import { createArrangementPropertiesRouter } from './arrangementProperties.js';
...
export function createArrangementsRouter(sessions, environment = process.env, dependencies = {}) {
    ...
    const limiter = () => dependencies.limiter ?? createDistributedRateLimiter(createPlatformRepository(undefined, environment), environment.PLATFORM_RATE_LIMIT_PEPPER?.trim() ?? '');
