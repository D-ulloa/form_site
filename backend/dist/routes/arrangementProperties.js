...
import { createArrangementPropertyRepository } from '../arrangements/arrangementPropertyRepository.js';
...
export function createArrangementPropertiesRouter(sessions, environment, dependencies) {
    ...
    async function actor(request, capability, policy) {
    ...
    function error(response, caught) {
        if (caught instanceof OrganizationDomainError)
        ...
        else if (caught instanceof z.ZodError || caught instanceof OrganizationValidationError)
    ...
    for (const [path, collection] of collections)
        router.get(path, async (request, response) => {
            try {
                const { scope, context } = await actor(request, collection === 'properties' ? 'arrangements.read' : 'arrangements.inquilinos.manage', collection === 'properties' ? 'arrangements.properties.read' : 'arrangements.inquilinos.read');
