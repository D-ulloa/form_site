...
import { InvitationActivationError } from '../identity/supabaseIdentityProvider.js';
...
export function createOrganizationGovernanceRouter(resolver, services, publicBaseUrl, rateLimiter) {
    ...
    router.post('/invitations/register', async (request, response) => {
        try {
            ...
            if (!registration?.registration_permitted)
                throw new OrganizationDomainError('INVITATION_INVALID');
            let stage = 'activate_account';
            try {
                await services.identityProvider.activateInvitationUser(registration.auth_user_id, registration.email_normalized, password, displayName);
                ...
                await services.invitations.completeRegistration(material[0], material[1], origin, identity.user_id, displayName, String(response.locals.request_id ?? ''));
                ...
                response.set('Set-Cookie', [...serializeSessionCookies(created.material, services.environment, created.session.remembered, created.max_age_seconds)]);
