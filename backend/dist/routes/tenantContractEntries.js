...
import { ContractEntryStateError } from '../services/contractEntryRepository.js';
...
import { createTenantContractHttpRepository, } from '../contracts/tenantContractHttpRepository.js';
...
function organizationParam(request) {
    return String(request.params.organization ?? "");
}
function requestId(response) {
    return String(response.locals.request_id ?? `req_${randomUUID()}`);
}
function publicBaseUrl(request, environment) {
...
function externalUrl(base, entryId, role, token) {
    ...
    router.post('/admin/entries/:entryId/tokens/:role/regenerate', async (request, response) => {
        try {
            ...
            await repository.replaceToken(scope, {
                user_id: context.user_id, membership_id: context.membership.id,
                request_id: requestId(response),
            }, entryId, current.version ?? 1, role, hashContractAccessToken(token, environment));
    ...
    const updateSubmission = async (request, response) => {
        try {
            ...
            const actor = { user_id: context.user_id, membership_id: context.membership.id,
                request_id: requestId(response) };
            const adapter = {
                updateRoleSubmission: async (input) => repository.appendRevision(scope, actor, entry, role, input.fields, input.submissionId),
            };
...
//# sourceMappingURL=tenantContractEntries.js.map