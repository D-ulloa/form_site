...
import { CONTRACT_EVIDENCE_FILE_MIME_TYPE_SET, ContractEvidenceUploadConfigurationError, ContractEvidenceUploadValidationError, ContractEvidenceVerificationUnavailableError, getContractEvidenceMaxFileBytes, issueContractEvidenceUploadUrls, issueContractEvidenceViewUrl, verifyContractEvidenceReferences, } from '../services/contractEvidenceUploadService.js';
import { buildContractAdminInspection, hydrateContractRoleValuesWithDownloadUrls, getContractSubmissionRecordsByRole, } from '../services/contractAdminInspectionService.js';
import { createContractSubmissionRateLimiter, } from '../services/contractSubmissionRateLimiter.js';
import { ContractTokenConfigurationError, verifyContractAccessToken, } from '../services/contractTokenService.js';
...
const EvidencePresignBodySchema = z.object({
    uploads: z.array(z.object({
        ...
        mimeType: z.string().refine((value) => CONTRACT_EVIDENCE_FILE_MIME_TYPE_SET.has(value), {
...
function resolveDependencies(overrides) {
    ...
    return {
        ...
        issueEvidenceUploadUrls: overrides.issueEvidenceUploadUrls ?? issueContractEvidenceUploadUrls,
        issueEvidenceViewUrl: overrides.issueEvidenceViewUrl ?? issueContractEvidenceViewUrl,
        verifyEvidenceReferences: overrides.verifyEvidenceReferences ?? verifyContractEvidenceReferences,
    };
}
...
function roleTokenHash(entry, role) {
    return role === 'user' ? entry.userTokenHash : entry.clientTokenHash;
}
function authorizeRoleAccess(req, entry, role, environment) {
...
async function loadEntry(entryIdValue, repository) {
    ...
    if (!entry)
        throw new ContractEntryNotFoundError(parsed.data);
    return entry;
}
...
function sendError(res, error) {
    ...
    if (error instanceof ContractDatabaseConfigurationError ||
...
export function createContractEntriesRouter(dependencyOverrides = {}) {
    ...
    const updateAdminRoleSubmission = async (req, res) => {
        setPrivateHeaders(res);
        try {
            const principal = authenticate(req, dependencies.environment);
            authorizeContractAdmin(principal, dependencies.environment);
    ...
    router.post('/:entryId/dni-uploads/presign', async (req, res) => {
        ...
        try {
            ...
            const uploads = await dependencies.issueDniUploadUrls(entry.id, body.uploads, dependencies.environment);
    ...
    router.post('/:entryId/evidence-uploads/presign', async (req, res) => {
        ...
        try {
            ...
            const rateLimit = dependencies.rateLimiter.check(`evidence:${normalizeContractRequestIp(req.ip)}:${entry.id}`);
            ...
            const uploads = await dependencies.issueEvidenceUploadUrls(entry.id, body.uploads, dependencies.environment);
            res.status(200).json({ uploads });
        }
        catch (error) {
            sendError(res, error);
    ...
    router.get('/:entryId/schema', async (req, res) => {
        ...
        try {
            ...
            const roleSchema = getContractRoleSchema(entry.schemaId, role, dependencies.environment);
            const values = role === 'user' ? entry.userSubmission : entry.clientSubmission;
            const downloadableValues = await hydrateContractRoleValuesWithDownloadUrls(entry, role, roleSchema.sections, values ?? {}, dependencies.environment, {
                issueDniViewUrl: dependencies.issueDniViewUrl,
                issueEvidenceViewUrl: dependencies.issueEvidenceViewUrl,
            });
            res.status(200).json({
    ...
    router.post('/:entryId/submit', async (req, res) => {
        ...
        try {
            const role = RoleSchema.parse(req.query.role);
            const entryId = EntryIdSchema.parse(req.params.entryId);
            const rateLimit = dependencies.rateLimiter.check(`${normalizeContractRequestIp(req.ip)}:${entryId}`);
            if (!rateLimit.allowed) {
                res.set('Retry-After', String(rateLimit.retryAfterSeconds));
                res.status(429).json({
                    error: 'RATE_LIMITED',
            ...
            const authorizedTokenHash = authorizeRoleAccess(req, entry, role, dependencies.environment);
...
//# sourceMappingURL=contractEntries.js.map