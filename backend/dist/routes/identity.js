...
function safeError(response, error) {
    ...
    if (error instanceof IdentityConfigurationError) {
        response.status(503).json({ error: 'AUTH_DEPENDENCY_UNAVAILABLE', retriable: true });
        return;
    }
...
function safeSelfServiceError(response, error) {
    privateHeaders(response);
...
function publicSession(session, memberships) {
...
function registrationInput(request, authMethod) {
...
async function rejectActiveSession(service, request) {
    try {
...
async function limitRegistration(limiter, request, email, operationId) {
    if (!limiter)
        return;
    let canonicalEmail;
    try {
        canonicalEmail = normalizeOrganizationEmail(email);
    }
    catch {
...
export function createIdentityRouter(service, provider, environment = process.env, onboarding, registrationRateLimiter) {
    const router = Router();
    ...
    router.post('/register', async (request, response) => {
        try {
            ...
            response.status(201).set('Set-Cookie', [...serializeSessionCookies(created.material, environment, created.session.remembered, created.max_age_seconds)]).json({
            ...
            safeSelfServiceError(response, error);
        }
    });
    ...
    router.post('/password/reset/request', async (request, response) => {
        try {
            ...
            if (email && email.length <= 320) {
                ...
                if (!redirect)
                    throw new IdentityConfigurationError('Password reset redirect is unavailable.');
                try {
                    await provider.requestPasswordReset(email, redirect);
                }
                catch { /* enumeration-resistant response */ }
            }
    ...
    router.post('/password/change', async (request, response) => {
        try {
            const authenticated = await service.authenticate(request, false);
            assertCsrf(request, authenticated.session.csrf_token_hash, environment);
            if (authenticated.session.assurance_level !== 'aal2')
    ...
    router.post('/login', async (request, response) => {
        try {
            ...
            response.set('Set-Cookie', [...serializeSessionCookies(created.material, environment, created.session.remembered, created.max_age_seconds)]);
    ...
    router.post('/sessions/rotate', async (request, response) => {
        try {
            ...
            response.set('Set-Cookie', [...serializeSessionCookies(rotated.material, environment, rotated.session.remembered, rotated.max_age_seconds)]);
...
export function createOrganizationContextRouter(service, repository, environment = process.env) {
    const router = Router();
    router.use((_request, response, next) => { privateHeaders(response); next(); });
    ...
    router.post('/organizations/:organizationId/api-keys', async (request, response) => {
        try {
            ...
            const result = await service.issueApiKey(request, String(request.params.organizationId ?? ''), {
                ...
                scopes: Array.isArray(body.scopes) ? body.scopes.filter((item) => typeof item === 'string') : [],
                expires_at: typeof body.expires_at === 'string' ? body.expires_at : '',
                allowed_ip_cidrs: Array.isArray(body.allowed_ip_cidrs)
                    ? body.allowed_ip_cidrs.filter((item) => typeof item === 'string') : [],
            });
    ...
    router.delete('/organizations/:organizationId/api-keys/:keyId', async (request, response) => {
        try {
            ...
            const context = await service.context(request, String(request.params.organizationId ?? ''), 'integrations.manage');
            const body = bodyRecord(request);
            await repository.revokeApiKey(context.organization.id, String(request.params.keyId ?? ''), context.membership.id, Number(body.expected_version), typeof body.reason_code === 'string' ? body.reason_code : 'manual_revoke', context.request_id);
            response.status(204).end();
        }
        catch (error) {
            safeError(response, error);
...
export function createTenantMutationSecurity(service, environment = process.env) {
    return (request, response, next) => {
        ...
        if (request.method === 'GET' || request.method === 'HEAD' || request.method === 'OPTIONS'
            ...
            || request.path === '/invitations/register' || usesContractAccessToken) {
            next();
            return;
        }
