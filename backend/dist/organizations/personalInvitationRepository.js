...
function record(data, error, organizationId) {
    if (error)
...
export function createPersonalInvitationRepository(environment = process.env, override) {
    const client = () => override ?? createPlatformServiceRoleClient(environment);
    const scope = (input) => ({ p_organization_id: input.organization_id, p_actor_membership_id: input.actor_membership_id });
    const token = (input) => ({ p_token_hash: input.token_hash, p_token_prefix: input.token_prefix,
...
//# sourceMappingURL=personalInvitationRepository.js.map