...
export class CaptureInvitationDeliveryAdapter {
    messages = [];
    async send(message) {
        this.messages.push(message);
        return { outcome: 'accepted_by_provider', provider_reference: `capture:${message.attempt_id}` };
    }
}
...
export class ResendInvitationDeliveryAdapter {
    apiKey;
    from;
    timeoutMilliseconds;
    fetcher;
    constructor(apiKey, from, timeoutMilliseconds, fetcher = fetch) {
        this.apiKey = apiKey;
        this.from = from;
    ...
    async send(message) {
        const rendered = renderInvitationEmail(message);
        ...
        try {
            ...
            if (!response.ok)
                return { outcome: response.status >= 500 || response.status === 429 ? 'ambiguous' : 'rejected',
                    safe_error_code: response.status === 429 ? 'PROVIDER_RATE_LIMITED' : 'PROVIDER_REJECTED' };
            const body = await response.text();
            if (Buffer.byteLength(body, 'utf8') > 16_384)
                return { outcome: 'ambiguous', safe_error_code: 'PROVIDER_RESPONSE_INVALID' };
            ...
            return { outcome: 'ambiguous', safe_error_code: 'PROVIDER_TIMEOUT_OR_UNAVAILABLE' };
        }
        finally {
            clearTimeout(timer);
        }
...
export function invitationDeliveryConfiguration(environment) {
    const enabled = environment.INVITATION_ROUTES_ENABLED === 'true';
    const deliveryMethod = (environment.INVITATION_DELIVERY_METHOD?.trim() || 'share_link');
    const adapter = (environment.INVITATION_EMAIL_ADAPTER?.trim() || 'disabled');
    ...
    if (!['share_link', 'email'].includes(deliveryMethod))
        throw new IdentityConfigurationError('Unknown invitation delivery method.');
    if (!['disabled', 'capture', 'resend'].includes(adapter))
        throw new IdentityConfigurationError('Unknown invitation adapter.');
    if (enabled) {
        let url;
        try {
            ...
            throw new IdentityConfigurationError('Invitation URL invalid.');
        }
        ...
        if (deliveryMethod === 'email') {
            ...
            if (adapter === 'disabled')
                throw new IdentityConfigurationError('Invitation delivery adapter is disabled.');
...
export function createInvitationDeliveryAdapter(environment) {
    const config = invitationDeliveryConfiguration(environment);
    if (config.adapter === 'capture')
        return new CaptureInvitationDeliveryAdapter();
    if (config.adapter === 'resend')
        return new ResendInvitationDeliveryAdapter(environment.RESEND_API_KEY ?? '', environment.INVITATION_EMAIL_FROM ?? '', Number(environment.INVITATION_EMAIL_TIMEOUT_MS ?? 5000));
    return new DisabledInvitationDeliveryAdapter();
}
export function verifyResendWebhook(payload, headers, secret, nowSeconds = Math.floor(Date.now() / 1000)) {
    const id = headers['svix-id'] ?? '';
    const timestamp = headers['svix-timestamp'] ?? '';
    const signatures = (headers['svix-signature'] ?? '').split(' ').map((part) => part.replace(/^v1,/u, ''));
    const seconds = Number(timestamp);
    if (!id || !Number.isSafeInteger(seconds) || Math.abs(nowSeconds - seconds) > 300)
        throw new Error('WEBHOOK_INVALID');
    const key = Buffer.from(secret.replace(/^whsec_/u, ''), 'base64');
    if (!secret.startsWith('whsec_') || key.length < 16)
        throw new Error('WEBHOOK_INVALID');
    ...
    if (!valid)
        throw new Error('WEBHOOK_INVALID');
    return { event_id: id, body: JSON.parse(payload.toString('utf8')) };
}
//# sourceMappingURL=invitationDelivery.js.map