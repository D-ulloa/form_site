...
export type IntegrationPurpose = 'property_export' | 'property_sheet' | 'property_events'
  | 'contract_sheet' | 'contract_generation';
export type IntegrationState = 'draft' | 'active' | 'disabled' | 'unhealthy' | 'rotating' | 'revoked';
export type DeliveryState = 'pending' | 'leased' | 'processing' | 'succeeded' | 'retry_wait'
  | 'reconciling' | 'unknown' | 'failed' | 'dead_letter' | 'blocked' | 'cancelled';
...
export type ProviderOutcome =
  | { readonly kind: 'succeeded'; readonly external_id: string; readonly receipt_reference?: string }
  | { readonly kind: 'transient_failure'; readonly error_code: string; readonly retry_after_seconds?: number }
  | { readonly kind: 'permanent_failure'; readonly error_code: string }
  | { readonly kind: 'ambiguous'; readonly error_code: string };
