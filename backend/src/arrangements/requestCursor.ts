...
export function createRequestCursorCodec(secret: string, binding: { organization_id: string; property_id: string | null; status: string | null; limit: number; audience?: string; membership_id?: string; ordering?: string; search?: string | null }) {
  if (Buffer.byteLength(secret) < 32) throw new PlatformError('DEPENDENCY_UNAVAILABLE');
  const hasSearch = Boolean(binding.search);
  const scope = JSON.stringify(['arrangements.orders', binding.audience ? (hasSearch ? 4 : 3) : (hasSearch ? 3 : 2),
    binding.organization_id, binding.property_id, binding.status, binding.limit,
    binding.ordering ?? 'submitted_at.desc.nullslast,id.desc', ...(binding.audience ? [binding.audience, binding.membership_id ?? null] : []),
    ...(hasSearch ? [binding.search] : [])]);
