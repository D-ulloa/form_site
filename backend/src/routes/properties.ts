...
import {
  ...
  type UploadMode,
} from '../services/createPropertySubmission.js';
import {
  issueSignedUploadUrls,
} from '../services/supabaseStorageService.js';
import {
  createUploadSession,
  consumeUploadSession,
} from '../services/mediaUploadSessionService.js';
import {
  validateMimeTypes,
  validateTotalSize,
  validateMediaUploadDescriptors,
  getUploadDescriptorTotalSize,
  MAX_MEDIA_FILES,
  MAX_UPLOAD_SIZE_BYTES,
} from '../utils/sizeLimits.js';
import type { MediaUploadMetadata } from '../types.js';
import {
...
function parseMediaUploads(raw: unknown): MediaUploadMetadata[] {
...
router.post('/media/presign', async (req, res) => {
  ...
  try {
    const presigned = await issueSignedUploadUrls(
      files.map((file) => ({
...
router.post('/submit', async (req, res) => {
  ...
  applyVerifiedPropertyActor(req.body as Record<string, unknown>, session);

  let media_uploads: MediaUploadMetadata[];
  let media_upload_session_id: string | undefined;
  try {
    media_uploads = parseMediaUploads((req.body as Record<string, unknown>).media_uploads);
    media_upload_session_id = parseMediaUploadSessionId((req.body as Record<string, unknown>).media_upload_session_id);
  } catch (err) {
  ...
  const result = await createPropertySubmission(
    validation.data,
    files,
    media_uploads,
    uploadMode,
