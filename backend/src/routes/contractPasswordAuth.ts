...
function authBody(body: unknown) {
...
function validateCredentials(credentials: ReturnType<typeof authBody>, registration: boolean): string | null {
  if (!credentials.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(credentials.email)) {
    return 'Ingresá un correo electrónico válido.';
  }
  if (credentials.password.length < 8) return 'La contraseña debe tener al menos 8 caracteres.';
  if (registration && !credentials.name) return 'Ingresá tu nombre completo.';
  return null;
}

function publicUser(session: Awaited<ReturnType<typeof loginContractUser>>) {
