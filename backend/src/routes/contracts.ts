...
} from '../services/contractAuditLogger.js';
import {
...
import {
  ...
  type ContractSubmissionReceipt,
  type CreateContractSubmissionInput,
} from '../services/createContractSubmission.js';
...
function setProtectedHeaders(
  ...
  if (audit) res.set('X-Content-Type-Options', 'nosniff');
}

function sendAuthenticationError(res: Response, error: unknown): boolean {
...
function logOperationalError(
  ...
  route: ContractRouteLogEntry['route'],
  status: number,
  error: unknown,
  requestId?: string,
): void {
...
function sendSubmitError(
  ...
  if (sendAuthenticationError(res, error)) return;
...
function validationErrors(result: Extract<ContractValidationResult, { success: false }>) {
...
export function createContractsRouter(
  dependencyOverrides: Partial<ContractsRouterDependencies> = {},
): Router {
  ...
  router.get('/schemas/:schemaId', (req, res) => {
    res.set('Cache-Control', 'no-store');
    try {
      const schema = dependencies.getPublicSchema(
        req.params.schemaId ?? '',
  ...
  router.post('/submit', async (req, res) => {
    const requestId = resolveContractRequestId(
    ...
    try {
      ...
      if (!validation.success) {
        const unknownSchema = validation.errors.some(
          (issue) => issue.code === 'unknown_schema',
        );
