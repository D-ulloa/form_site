...
export function createSafeMetrics(sink: MetricsSink) {
  return {
    async record(name: string, value: number, labels: Readonly<Record<string, string>> = {}): Promise<void> {
      if (!METRIC_NAME.test(name) || !Number.isFinite(value)) throw new Error('INVALID_METRIC');
