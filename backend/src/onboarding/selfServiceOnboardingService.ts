...
import type { SelfServiceIdentityAdminAdapter, ProvisioningAuthUser } from '../identity/supabaseAdminAdapter.js';
...
import {
  SelfServiceOnboardingError, type SelfServiceOnboardingOperation, type SelfServicePasswordRegistrationInput,
  type SelfServiceRegistrationInput,
} from './selfServiceOnboardingTypes.js';
...
function operationPayload(input: SelfServiceRegistrationInput, email: string): string {
...
export class SelfServiceOnboardingService {
  constructor(
    private readonly repository: SelfServiceOnboardingRepository,
    private readonly identities: SelfServiceIdentityAdminAdapter,
  ...
  private defaults() {
    ...
    if (!pepper || Buffer.byteLength(pepper, 'utf8') < 32) throw new SelfServiceOnboardingError('REGISTRATION_DISABLED');
    return {
      pepper,
  ...
  private async claim(input: SelfServiceRegistrationInput, incomingRequestId?: string): Promise<{ operation: SelfServiceOnboardingOperation; email: string }> {
    this.assertEnabled();
    if (!UUID.test(input.operation_id) || input.terms_accepted !== true) throw new SelfServiceOnboardingError('INVALID_REQUEST');
  ...
  private async establish(operation: SelfServiceOnboardingOperation, identity: CreatedSelfServiceIdentity, incomingRequestId?: string) {
    ...
    const marked = operation.auth_user_id ? operation : await this.repository.markIdentity(
      operation.operation_id, identity.user_id, emailFingerprint,
      identity.auth_method === 'google' ? 'google' : 'password', id,
    );
    if (marked.auth_user_id !== identity.user_id) throw new SelfServiceOnboardingError('FORBIDDEN');
    const completed = await this.repository.complete(marked.operation_id, identity.user_id, id);
    if (completed.state !== 'completed') throw new SelfServiceOnboardingError('ONBOARDING_RECOVERY_REQUIRED');
    return completed;
  }

  async registerPassword(input: SelfServicePasswordRegistrationInput, incomingRequestId?: string) {
    ...
    if (claimed.operation.state === 'rejected') throw new SelfServiceOnboardingError('EXISTING_ACCOUNT');
    ...
    let matches: readonly ProvisioningAuthUser[];
    try { matches = await this.identities.resolveByEmail(claimed.email); }
    catch { throw new SelfServiceOnboardingError('AUTH_DEPENDENCY_UNAVAILABLE'); }
    if (matches.length > 1) {
    ...
    let identity: CreatedSelfServiceIdentity;
    try {
      ...
      try {
        ...
        } else throw new SelfServiceOnboardingError('AUTH_DEPENDENCY_UNAVAILABLE');
      } catch (error) {
        if (error instanceof SelfServiceOnboardingError) throw error;
  ...
  async startGoogle(input: SelfServiceRegistrationInput, incomingRequestId?: string): Promise<SelfServiceOnboardingOperation> {
    const claimed = await this.claim(input, incomingRequestId);
    if (claimed.operation.state === 'rejected') throw new SelfServiceOnboardingError('EXISTING_ACCOUNT');
    try {
  ...
  async completeGoogle(operationId: string, identity: CreatedSelfServiceIdentity, incomingRequestId?: string) {
    this.assertEnabled();
    if (!UUID.test(operationId)) throw new SelfServiceOnboardingError('INVALID_REQUEST');
    const operation = await this.repository.get(operationId, identity.user_id).catch(async (error: unknown) => {
      if (!(error instanceof SelfServiceOnboardingError) || error.code !== 'FORBIDDEN') throw error;
      // The authenticated Google identity is bound on its first callback only.
      return null;
    });
    if (operation) return { identity, operation: await this.establish(operation, identity, incomingRequestId) };
  ...
  async recover(operationId: string, identity: CreatedSelfServiceIdentity, incomingRequestId?: string) {
    this.assertEnabled();
