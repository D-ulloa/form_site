...
import { FEATURE_STATES, MIGRATION_MODES, type CertificationDecision, type CertificationInput,
  type InventoryDecision, type InventoryDecisionInput, type MigrationManifest,
  type RollbackThreshold, type SolarRolloutStage } from './types.js';
...
const REQUIRED_APPROVAL_ROLES = ['security', 'product', 'data', 'backend', 'frontend', 'operations',
  'provider', 'support', 'release'] as const;
...
function assertNoSecretMaterial(value: unknown, path = 'manifest'): void {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (SECRET_KEY.test(key)) throw new Error(`SECRET_MATERIAL_FORBIDDEN:${path}.${key}`);
    assertNoSecretMaterial(child, `${path}.${key}`);
  }
}

export function validateMigrationManifest(manifest: MigrationManifest): void {
  ...
  if (!manifest || typeof manifest !== 'object' || !manifest.azar || !manifest.solar
    ...
    || !Array.isArray(manifest.approval_references)) throw new Error('INVALID_MIGRATION_MANIFEST');
  if (!MIGRATION_MODES.includes(manifest.mode)) throw new Error('INVALID_MIGRATION_MODE');
  ...
  if (!UUID.test(manifest.azar.organization_id) || !UUID.test(manifest.solar.organization_id)
    || manifest.azar.organization_id === manifest.solar.organization_id) throw new Error('INVALID_FIXED_ORGANIZATION_IDS');
  if (!SLUG.test(manifest.azar.slug) || !SLUG.test(manifest.solar.slug)
    || manifest.azar.slug === manifest.solar.slug) throw new Error('INVALID_FIXED_ORGANIZATION_SLUGS');

  const features = new Set<string>();
  for (const feature of manifest.solar_features) {
    if (!KEY.test(feature.feature_key) || features.has(feature.feature_key)
      || !FEATURE_STATES.includes(feature.state)) throw new Error('INVALID_SOLAR_FEATURE_MANIFEST');
  ...
  for (const threshold of manifest.rollback_thresholds) {
    ...
    if (metrics.has(threshold.metric_key)) throw new Error('DUPLICATE_ROLLBACK_THRESHOLD');
    metrics.add(threshold.metric_key);
  }
  for (const reference of manifest.approval_references) requireText(reference, 'INVALID_APPROVAL_REFERENCE');
  if (manifest.mode === 'production' && (manifest.approval_references.length === 0
    || manifest.rollback_thresholds.length === 0)) throw new Error('PRODUCTION_GATE_INCOMPLETE');
}

function validateThreshold(threshold: RollbackThreshold): void {
...
export function decideInventoryDisposition(input: InventoryDecisionInput): InventoryDecision {
  if (!HASH.test(input.source_fingerprint)) return {
  ...
  if (input.proposed_disposition === 'migrate_to_azar' && (!input.ownership_evidence_reference?.trim()
    || !input.approved_rule_reference?.trim() || !input.reviewer_id?.trim())) return {
  ...
  if (input.proposed_disposition === 'delete_after_approval'
    && (!input.retention_approved || input.legal_hold || !input.reviewer_id?.trim())) return {
...
export function evaluateReleaseCertification(input: CertificationInput): CertificationDecision {
  const blockers: string[] = [];
  if (!input.artifact_match) blockers.push('DEPLOYED_ARTIFACT_MISMATCH');
  if (!input.provider_destinations_distinct) blockers.push('PROVIDER_DESTINATIONS_NOT_DISTINCT');
  if (!input.restore_rehearsal_passed) blockers.push('RESTORE_REHEARSAL_MISSING');
  if (!input.migration_rehearsal_passed) blockers.push('MIGRATION_REHEARSAL_MISSING');
  ...
  if (input.features.some((feature) => feature.state === 'certified_enabled'
    && !feature.certification_reference?.trim())) blockers.push('FEATURE_CERTIFICATION_MISSING');
  if (input.thresholds.length === 0) blockers.push('ROLLBACK_THRESHOLDS_MISSING');
  else {
    try { input.thresholds.forEach(validateThreshold); } catch { blockers.push('ROLLBACK_THRESHOLDS_INVALID'); }
  }
  const roles = new Set(input.approval_roles);
  if (REQUIRED_APPROVAL_ROLES.some((role) => !roles.has(role))) blockers.push('GO_NO_GO_APPROVALS_INCOMPLETE');
  return { releasable: blockers.length === 0, blockers: Object.freeze(blockers) };
}

const rolloutTransitions: Readonly<Record<SolarRolloutStage, ReadonlySet<SolarRolloutStage>>> = {
...
export function assertSolarRolloutTransition(from: SolarRolloutStage, to: SolarRolloutStage,
  certification: CertificationDecision, boundaryIncident = false): void {
  if (boundaryIncident && to !== 'contained') throw new Error('BOUNDARY_INCIDENT_REQUIRES_CONTAINMENT');
  if (!rolloutTransitions[from].has(to)) throw new Error('INVALID_SOLAR_ROLLOUT_TRANSITION');
