...
interface ContractEntryRow {
  readonly id: string; readonly organization_id: string; readonly schema_id: string;
  readonly direccion: string | null; readonly created_by: string;
  readonly created_by_user_id: string | null; readonly created_at: string;
  readonly user_token_hash: string; readonly client_token_hash: string;
  readonly user_filled: boolean; readonly client_filled: boolean;
  readonly user_submitted_at: string | null; readonly client_submitted_at: string | null;
  readonly user_submission: Readonly<Record<string, unknown>> | null;
  readonly client_submission: Readonly<Record<string, unknown>> | null;
  readonly combined_submission: Readonly<Record<string, unknown>> | null;
  readonly status: ContractEntryRecord['status']; readonly archived_at: string | null;
...
function failure(error: { message: string } | null, data?: unknown): never {
  const message = error?.message ?? (data ? 'DATABASE_FAILURE' : 'NO_DATA');
  if (message.includes('NOT_FOUND')) throw new PlatformError('NOT_FOUND');
  if (message.includes('FORBIDDEN')) throw new PlatformError('FORBIDDEN');
...
export interface TenantContractHttpRepository {
  create(scope: OrganizationScope, actor: TenantActor, input: {
    readonly id: string; readonly schema_id: string; readonly direccion: string;
    readonly user_token_hash: string; readonly client_token_hash: string;
  ...
  setStatus(scope: OrganizationScope, actor: TenantActor, entryId: string,
    expectedVersion: number, status: 'open' | 'complete' | 'generar_contrato'): Promise<ContractEntryRecord>;
  archive(scope: OrganizationScope, actor: TenantActor, entryId: string,
    expectedVersion: number): Promise<ContractEntryRecord>;
  replaceToken(scope: OrganizationScope, actor: TenantActor, entryId: string,
    expectedVersion: number, role: ContractRole, tokenHash: string): Promise<ContractEntryRecord>;
  appendRevision(scope: OrganizationScope, actor: TenantActor, entry: ContractEntryRecord,
    role: ContractRole, fields: Readonly<Record<string, unknown>>,
    idempotencyKey: string): Promise<ContractEntryRecord>;
}

export function createTenantContractHttpRepository(
  clientOverride?: SupabaseClient,
  environment: NodeJS.ProcessEnv = process.env,
): TenantContractHttpRepository {
  const client = () => clientOverride ?? createPlatformServiceRoleClient(environment);
  const find = async (scope: OrganizationScope, entryId: string) => {
    ...
    return data ? toEntry(data as unknown as ContractEntryRow) : null;
  };
  return {
    async create(scope, actor, input) {
      ...
      if (error || !data) failure(error, data);
      return toEntry(data as unknown as ContractEntryRow);
    },
    async list(scope) {
      const { data, error } = await client().from('contract_entries').select('*')
      ...
      if (error) failure(error);
      return (data ?? []).map((row) => toEntry(row as unknown as ContractEntryRow));
    },
    find,
    async submissions(scope, entryId) {
      if (!await find(scope, entryId)) throw new PlatformError('NOT_FOUND');
      ...
      if (error) failure(error);
      return (data ?? []).map((row) => ({
        id: String(row.id), entryId: String(row.entry_id), role: row.role as ContractRole,
        submission: row.submission as Readonly<Record<string, unknown>>,
        metadata: row.submission_meta as ContractSubmissionRecord['metadata'],
