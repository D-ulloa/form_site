 ...
 */
export function mapContractFieldsToSheetRow(
  ...
  if (expectedNames.size !== fieldNames.length) {
    throw new ContractMappingError(
      `Schema ${schema.schemaId} contains duplicate field names.`,
    );
  }

  const unexpectedMappings = Object.keys(schema.columnMap).filter(
    (fieldName) => !expectedNames.has(fieldName),
  );
  if (unexpectedMappings.length > 0) {
    throw new ContractMappingError(
      `Schema ${schema.schemaId} maps unknown fields: ${unexpectedMappings.join(', ')}.`,
    );
  }
  ...
  if (unexpectedFields.length > 0) {
    throw new ContractMappingError(
      `Submission contains unmapped fields: ${unexpectedFields.join(', ')}.`,
    );
  }

  const columnHeaders: string[] = [];
  const values: ContractFieldValue[] = [];

  for (const field of definitions) {
    const header = schema.columnMap[field.name]?.trim();
    if (!header) {
      throw new ContractMappingError(
        `Schema ${schema.schemaId} has no Sheet column mapping for ${field.name}.`,
      );
    }
    ...
    if (!Object.prototype.hasOwnProperty.call(fields, field.name)) {
      if (field.required) {
        throw new ContractMappingError(
          `Validated submission is missing required field ${field.name}.`,
        );
      }
    ...
    if (value === undefined) {
      throw new ContractMappingError(
        `Validated submission has an undefined value for ${field.name}.`,
      );
    }
