...
import {
  createPlatformProvisioningActor, IdentityProvisioningError, type IdentityProvisioningPurpose,
...
function argumentsByName(values: readonly string[]): ReadonlyMap<string, string> {
  const result = new Map<string, string>();
  for (let index = 0; index < values.length; index += 2) {
    const name = values[index]; const value = values[index + 1];
    if (!name?.startsWith('--') || !value || value.startsWith('--')) throw new Error('INVALID_COMMAND_ARGUMENTS');
    result.set(name.slice(2), value);
  }
  return result;
}

export async function runProvisionIdentityCommand(
  ...
  if (purpose !== 'initial_owner' && purpose !== 'organization_invitee') throw new Error('INVALID_PURPOSE');
  const required = (name: string): string => {
    const value = args.get(name)?.trim(); if (!value) throw new Error(`MISSING_${name.toUpperCase().replaceAll('-', '_')}`); return value;
  };
  const service = new IdentityProvisioningService(
    createIdentityProvisioningRepository(environment), createSupabaseAdminAdapter(environment), environment,
  );
  const result = await service.provision({
    email: required('email'), purpose: purpose as IdentityProvisioningPurpose,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runProvisionIdentityCommand().catch((error: unknown) => {
