...
export class InvitationActivationError extends Error {
  constructor(readonly code: 'PASSWORD_POLICY_REJECTED' | 'ACCOUNT_ALREADY_ACTIVATED' | 'AUTH_DEPENDENCY_UNAVAILABLE',
    readonly providerCode: string = 'unknown') {
    super(code);
    this.name = 'InvitationActivationError';
  }
}

function activationProviderError(error: { code?: string | undefined }): InvitationActivationError {
...
export function createSupabaseIdentityProvider(environment: NodeJS.ProcessEnv = process.env): IdentityProvider {
  ...
  return {
    ...
    async activateInvitationUser(userId, expectedEmail, password, displayName) {
      ...
      if (error) throw activationProviderError(error);
      ...
      if (updateError) throw activationProviderError(updateError);
    },
  };
}
