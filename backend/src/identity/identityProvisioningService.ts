...
import type { IdentityAdminAdapter, ProvisioningAuthUser } from './supabaseAdminAdapter.js';
import { IdentityProviderAmbiguousError, IdentityProviderUnavailableError } from './supabaseAdminAdapter.js';
import type { IdentityProvisioningRepository, ProvisioningOperationRecord } from './identityProvisioningRepository.js';
import type {
  IdentityProvisioningActor, IdentityProvisioningOutcome, ProvisionIdentityInput, ProvisionIdentityResult,
} from './identityProvisioningTypes.js';
import { IdentityProvisioningError } from './identityProvisioningTypes.js';
import {
  normalizeOrganizationEmail, validateDisplayName, validateLocale, validateTimeZone,
...
function stableFingerprint(value: Readonly<Record<string, string>>): string {
...
function result(
  ...
  email: string,
  idempotencyOverride?: ProvisionIdentityResult['idempotency'],
): ProvisionIdentityResult {
  if (!operation.outcome) throw new IdentityProvisioningError('AUDIT_UNAVAILABLE');
  return {
...
export class IdentityProvisioningService {
  constructor(
    private readonly repository: IdentityProvisioningRepository,
    private readonly admin: IdentityAdminAdapter,
    private readonly environment: NodeJS.ProcessEnv = process.env,
  ) {}

  async provision(input: ProvisionIdentityInput, actor: IdentityProvisioningActor): Promise<ProvisionIdentityResult> {
    ...
    if (!pepper || Buffer.byteLength(pepper, 'utf8') < 32) throw new IdentityProvisioningError('PROVISIONING_DISABLED');
    ...
    if (operation.claim_state === 'busy') throw new IdentityProvisioningError('PROVISIONING_IN_PROGRESS');
    if (operation.claim_state === 'replayed') return result(operation, email);
    ...
    let user = matches[0];
    let outcome: Exclude<IdentityProvisioningOutcome, 'blocked_ambiguous' | 'blocked_ineligible'>;
    if (user) {
      if (!user.eligible) {
        const blocked = await this.repository.block({ operation_id: operation.operation_id,
      ...
      try {
        ...
        if (user.email_normalized !== email) throw new IdentityProviderAmbiguousError();
        ...
        try {
          ...
          if (reconciled.length === 0) throw new IdentityProvisioningError('IDENTITY_PROVIDER_UNAVAILABLE');
          [user] = reconciled;
          if (!user?.eligible) {
            const blocked = await this.repository.block({ operation_id: operation.operation_id,
              outcome: 'blocked_ineligible', reason_code: 'IDENTITY_INELIGIBLE', request_id: input.request_id,
          ...
          if (reconciliationError instanceof IdentityProvisioningError) throw reconciliationError;
          throw new IdentityProvisioningError('IDENTITY_PROVIDER_UNAVAILABLE');
        }
      }
    }
    if (!user) throw new IdentityProvisioningError('IDENTITY_PROVIDER_UNAVAILABLE');
