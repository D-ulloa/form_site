...
import type { IdentityRepository, SessionCreateInput } from './identityRepository.js';
import {
  IdentityAccessError, createSessionTokenMaterial, hashSessionSecret,
  secretMatches, sessionTokenFromRequest,
} from './sessionSecurity.js';
import type {
  AppSessionRecord, OrganizationApiKeyRecord, OrganizationRequestContext,
  OrganizationApiKeyContext, SessionIdentity, SessionMembershipSummary, SessionTokenMaterial,
} from './types.js';
...
function positiveSeconds(environment: NodeJS.ProcessEnv, key: string, fallback: number): number {
  const raw = environment[key]?.trim();
  if (!raw) return fallback;
  const value = Number(raw);
  if (!Number.isSafeInteger(value) || value < 60) throw new Error(`${key} must be an integer of at least 60 seconds.`);
  return value;
}

function activeSessionLimit(environment: NodeJS.ProcessEnv): number {
  const value = Number(environment.APP_MAX_ACTIVE_SESSIONS ?? 10);
  if (!Number.isSafeInteger(value) || value < 1 || value > 100) throw new Error('APP_MAX_ACTIVE_SESSIONS must be between 1 and 100.');
  return value;
}
...
export function ipMatchesRestriction(ip: string, restriction: string): boolean {
  const normalized = ip.startsWith('::ffff:') ? ip.slice(7) : ip;
  const [network = '', prefixRaw] = restriction.split('/');
  if (prefixRaw === undefined) return normalized === network;
  const prefix = Number(prefixRaw);
  const addressValue = ipv4Number(normalized); const networkValue = ipv4Number(network);
...
export class SessionService {
  ...
  async authenticate(request: Request, touch = true): Promise<{ session: AppSessionRecord; identity: Awaited<ReturnType<IdentityRepository['getUser']>> & {} }> {
    if (request.get('Authorization')) throw new IdentityAccessError('AMBIGUOUS_CREDENTIALS', 401);
    const raw = sessionTokenFromRequest(request, this.environment);
    if (!raw || raw.length < 32 || raw.length > 128) throw new IdentityAccessError('AUTHENTICATION_REQUIRED', 401);
    ...
    if (!identity) throw new IdentityAccessError('SESSION_INVALID', 401);
    let current = session;
  ...
  async apiKeyContext(request: Request, organizationId: string, requiredScope: string): Promise<OrganizationApiKeyContext> {
    if (sessionTokenFromRequest(request, this.environment)) throw new IdentityAccessError('AMBIGUOUS_CREDENTIALS', 401);
    const match = /^Bearer[ \t]+(org_[A-Za-z0-9_-]{8,24})\.([A-Za-z0-9_-]{40,64})$/u.exec(request.get('Authorization')?.trim() ?? '');
    if (!match?.[1] || !match[2]) throw new IdentityAccessError('AUTHENTICATION_REQUIRED', 401);
    const pepper = this.environment.APP_API_KEY_PEPPER?.trim();
    if (!pepper || Buffer.byteLength(pepper) < 32) throw new Error('API key verifier is unavailable.');
  ...
  async memberships(userId: string): Promise<readonly SessionMembershipSummary[]> {
  ...
  async context(request: Request, organizationIdOrSlug: string, capability?: OrganizationCapability, touch = true): Promise<OrganizationRequestContext> {
  ...
  async rotate(request: Request): Promise<{ session: AppSessionRecord; material: SessionTokenMaterial; max_age_seconds: number }> {
    const { session } = await this.authenticate(request, false);
    const material = createSessionTokenMaterial(this.environment);
    ...
    const next = await this.repository.rotateSession(session, material, session.absolute_expires_at,
      new Date(this.now().getTime() + idleSeconds * 1000).toISOString(), requestId(request));
    return { session: next, material, max_age_seconds: maxAge };
  }

  async revokeOthers(request: Request): Promise<number> {
    const { session } = await this.authenticate(request, false);
    return this.repository.revokeOtherSessions(session, requestId(request));
  }

  async listSessions(request: Request): Promise<readonly AppSessionRecord[]> {
    const { session } = await this.authenticate(request);
    return this.repository.listUserSessions(session.user_id);
  }

  createApiKeyMaterial(): { raw: string; prefix: string; hash: string } {
    const rawSecret = randomBytes(32).toString('base64url');
    const prefix = `org_${rawSecret.slice(0, 12)}`;
    const pepper = this.environment.APP_API_KEY_PEPPER?.trim();
    if (!pepper || Buffer.byteLength(pepper) < 32) throw new Error('APP_API_KEY_PEPPER must contain at least 32 bytes.');
    return { raw: `${prefix}.${rawSecret}`, prefix,
      hash: createHmac('sha256', pepper).update(`${prefix}.${rawSecret}`).digest('hex') };
  }

  async issueApiKey(request: Request, organizationId: string, input: {
    name: string; scopes: readonly string[]; expires_at: string; allowed_ip_cidrs?: readonly string[];
  }): Promise<{ raw_key: string; key: OrganizationApiKeyRecord }> {
    const context = await this.context(request, organizationId, 'integrations.manage');
    if (context.assurance_level !== 'aal2') throw new IdentityAccessError('STEP_UP_REQUIRED', 403);
