...
export function identityProvisioningDefaults(environment: NodeJS.ProcessEnv): IdentityProvisioningDefaults {
  return {
...
export function validateIdentityProvisioningEnvironment(environment: NodeJS.ProcessEnv): void {
  ...
  for (const key of ['IDENTITY_PROVISIONING_DEFAULT_DISPLAY_NAME', 'IDENTITY_PROVISIONING_DEFAULT_LOCALE',
    ...
    if (!environment[key]?.trim()) throw new IdentityConfigurationError(`${key} is required in production.`);
  }
