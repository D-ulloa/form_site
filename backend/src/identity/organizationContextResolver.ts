...
import type { OrganizationRouteContextResolver } from '../routes/organizationGovernance.js';
import type { SessionService } from './sessionService.js';

function routeOrganization(request: Request): string {
  const value = request.params.organizationId ?? request.params.organization;
  return Array.isArray(value) ? value[0] ?? '' : value ?? '';
}

export function createOrganizationRouteContextResolver(service: SessionService): OrganizationRouteContextResolver {
  return {
