...
import { createArrangementRequestRepository } from '../../src/arrangements/requestRepository.js';
import { createArrangementRequestsService } from '../../src/services/arrangementRequests.js';
import { createAssetRepository } from '../../src/assets/assetRepository.js';
import { createFixtureStorage } from './spec43-storage.js';
...
import { createIdentityProvisioningRepository } from '../../src/identity/identityProvisioningRepository.js';
import { createPersonalInvitationRepository } from '../../src/organizations/personalInvitationRepository.js';
...
const endpoint = new URL(process.env.SPEC45_POSTGREST_URL ?? 'http://127.0.0.1:55444');
if (!['127.0.0.1', 'localhost'].includes(endpoint.hostname)) throw new Error('Disposable loopback database required');
const secret = process.env.SPEC45_TEST_JWT_SECRET;
if (!secret || secret.length < 32) throw new Error('SPEC45_TEST_JWT_SECRET required');
const origin = 'http://127.0.0.1:4173';
const environment = { NODE_ENV: 'test', ARRANGEMENT_REJECTION_ENABLED: 'true', PERSONAL_INVITATIONS_ENABLED: 'true', IDENTITY_PROVISIONING_ENABLED: 'true', IDENTITY_PROVISIONING_EMAIL_PEPPER: 'e'.repeat(48), APP_SESSION_PEPPER: 's'.repeat(48), APP_CSRF_PEPPER: 'c'.repeat(48), APP_ALLOWED_ORIGINS: origin, PLATFORM_CURSOR_SECRET: 'q'.repeat(48), PLATFORM_RATE_LIMIT_PEPPER: 'r'.repeat(48) };
...
const names = ['owner', 'admin', 'other', 'solar-owner', 'inquilino', 'second', 'member', 'viewer', 'personal', 'tenant', 'new-personal', 'existing-personal', 'personal-two', 'personal-b', 'co-tenant'];
...
const passwords = new Map(users.filter(user => !['inquilino@example.test','second@example.test','new-personal@example.test'].includes(user.email)).map(user => [user.id, 'spec45-test-password']));
...
const provider = {
  ...
  async activateInvitationUser(id: string, email: string, password: string, displayName: string) {
    ...
    const { error } = await client.rpc('spec44_fixture_auth_user', { p_id:id, p_email:email, p_activate:true });
    if (error) throw error;
    passwords.set(id, password); user.display_name = displayName;
  },
  async accessToken(token: string) {
    const user = users.find(candidate => token === `spec45-google:${candidate.email}`);
    if (!user) throw new Error('INVALID_CREDENTIALS');
    return { user_id: user.id, email: user.email, display_name: user.display_name, auth_method: 'google', assurance_level: 'aal1' };
  },
} as IdentityProvider;
...
const provisioning = new IdentityProvisioningService(createIdentityProvisioningRepository(environment, client), {
  async resolveByEmail(email) {
    return users.filter(user => user.email === email).map(user => ({ id:user.id,email_normalized:email,
      activation_required:!passwords.has(user.id),eligible:true }));
  },
  async createInviteOnly(email) {
    const id=randomUUID();
    const {error}=await client.rpc('spec44_fixture_auth_user',{p_id:id,p_email:email,p_activate:false});
    if(error) throw error;
    users.push({id,email,display_name:'Usuario invitado'});
    return {id,email_normalized:email,activation_required:true,eligible:true};
  },
},environment);
const services = {
  organizations: new OrganizationService(createOrganizationGovernanceRepository(environment, client), undefined, invitations, provisioning, createPersonalInvitationRepository(environment,client), environment),
...
const fixtureStorage = createFixtureStorage(app);
...
app.use('/api/organizations/:organization/arrangements', createArrangementsRouter(sessions, environment, {
  requests: createArrangementRequestsService(createArrangementRequestRepository(client, environment), { storage: fixtureStorage.storage, detectContent: fixtureStorage.detectContent, recordUrlIssued: createAssetRepository(client, environment).recordUrlIssued }, environment),
  properties: createArrangementPropertiesService(createArrangementPropertyRepository(client, environment), environment),
  organizations: services.organizations, personal: services.organizations, limiter,
}));
...
app.listen(Number(process.env.SPEC45_API_PORT ?? 3002), '127.0.0.1', () => process.stdout.write('SPEC-45 local API on 127.0.0.1:3002\n'));
