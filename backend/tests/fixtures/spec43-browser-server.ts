...
const endpoint = new URL(process.env.SPEC43_POSTGREST_URL ?? 'http://127.0.0.1:55444');
if (!['127.0.0.1', 'localhost'].includes(endpoint.hostname)) throw new Error('Disposable loopback database required');
const secret = process.env.SPEC43_TEST_JWT_SECRET;
if (!secret || secret.length < 32) throw new Error('SPEC43_TEST_JWT_SECRET required');
...
const names = ['owner', 'admin', 'other', 'solar-owner', 'inquilino', 'second', 'different', 'reader', 'viewer'];
...
const passwords = new Map(users.map(user => [user.id, 'spec43-test-password']));
...
app.listen(3002, '127.0.0.1', () => process.stdout.write('SPEC-43 local API on 127.0.0.1:3002\n'));
