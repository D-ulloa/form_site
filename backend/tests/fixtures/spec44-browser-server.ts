...
const endpoint = new URL(process.env.SPEC44_POSTGREST_URL ?? 'http://127.0.0.1:55444');
if (!['127.0.0.1', 'localhost'].includes(endpoint.hostname)) throw new Error('Disposable loopback database required');
const secret = process.env.SPEC44_TEST_JWT_SECRET;
if (!secret || secret.length < 32) throw new Error('SPEC44_TEST_JWT_SECRET required');
const origin = 'http://127.0.0.1:4173';
const environment = { NODE_ENV: 'test', PERSONAL_INVITATIONS_ENABLED: 'true', IDENTITY_PROVISIONING_ENABLED: 'true', IDENTITY_PROVISIONING_EMAIL_PEPPER: 'e'.repeat(48), APP_SESSION_PEPPER: 's'.repeat(48), APP_CSRF_PEPPER: 'c'.repeat(48), APP_ALLOWED_ORIGINS: origin, PLATFORM_CURSOR_SECRET: 'q'.repeat(48), PLATFORM_RATE_LIMIT_PEPPER: 'r'.repeat(48) };
...
const names = ['owner', 'admin', 'other', 'solar-owner', 'inquilino', 'second', 'member', 'viewer', 'personal', 'tenant', 'new-personal', 'existing-personal'];
...
const passwords = new Map(users.filter(user => !['inquilino@example.test','second@example.test','new-personal@example.test'].includes(user.email)).map(user => [user.id, 'spec44-test-password']));
...
app.listen(3002, '127.0.0.1', () => process.stdout.write('SPEC-44 local API on 127.0.0.1:3002\n'));
